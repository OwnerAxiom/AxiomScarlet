...
from AxiomMuzic.plugins.tools.pretenderdb import (
    impo_off, impo_on, check_pretender,
    add_userdata, get_userdata, usr_data
)
from AxiomMuzic.utils.admin_filters import admin_filter
from config import MAANAV_IMG
...
""".format(
    ...
    if msg != "":
        await message.reply_photo(random. choice(MAANAV_IMG), caption=msg)


@app.on_message(filters.group & filters.command("axiomchk") & ~filters.bot & ~filters.via_bot & admin_filter)
async def set_mataa(_, message: Message):
    if len(message.command) == 1:
        return await message.reply("ᴅᴇᴛᴇᴄᴛ ᴘʀᴇᴛᴇɴᴅᴇʀ ᴜsᴇʀs **ᴜsᴀɢᴇ:** `/axiomchk enable|disable`")
